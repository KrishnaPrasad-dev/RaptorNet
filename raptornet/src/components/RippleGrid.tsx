...
type UniformMap = Record<string, ShaderUniform>;
...
`;
      ...
      const render = (time: number) => {
        ...
        mousePosition.x += (mouseTarget.x - mousePosition.x) * 0.1;
        mousePosition.y += (mouseTarget.y - mousePosition.y) * 0.1;

        const influenceValue = uniforms.mouseInfluence.value as number;
        uniforms.mouseInfluence.value =
          influenceValue + (mouseInfluenceTarget - influenceValue) * 0.05;
      ...
      return () => {
        ...
        try {
          ...
          // Ignore cleanup failures to avoid breaking route transitions.
        }
      };
    } catch (error) {
      console.error("RippleGrid failed to initialize:", error);
      return;
    }
  }, [
    enableRainbow,
